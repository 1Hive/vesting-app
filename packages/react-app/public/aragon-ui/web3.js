...
var providerTypes = require('./provider-types.js');
...
var BLOCK_EXPLORERS = {
  etherscan: function etherscan(_ref) {
    ...
    if (!providerTypes.ETHERSCAN_NETWORK_TYPES.has(networkType)) {
      throw new Error('provider not supported.');
    }

    if (!providerTypes.ETHERSCAN_TYPES.has(type)) {
      throw new Error('type not supported.');
    }

    var subdomain = providerTypes.ETHERSCAN_NETWORK_TYPES.get(networkType);
    var typePart = providerTypes.ETHERSCAN_TYPES.get(type);
    return "https://".concat(subdomain, "etherscan.io/").concat(typePart, "/").concat(value);
  },
  blockscout: function blockscout(_ref2) {
    var type = _ref2.type,
        value = _ref2.value,
    ...
    if (!providerTypes.BLOCKSCOUT_NETWORK_TYPES.has(networkType)) {
      throw new Error('provider not supported.');
    }

    if (!providerTypes.BLOCKSCOUT_TYPES.has(type)) {
      throw new Error('type not supported.');
    }

    var networkName = providerTypes.BLOCKSCOUT_NETWORK_TYPES.get(networkType);
    var typePart = providerTypes.BLOCKSCOUT_TYPES.get(type);
    return "https://blockscout.com/poa/".concat(networkName, "/").concat(typePart, "/").concat(value);
  },
  polygonscan: function polygonscan(_ref3) {
    var type = _ref3.type,
        value = _ref3.value,
    ...
    if (!providerTypes.POLYGONSCAN_NETWORK_TYPES.has(networkType)) {
      throw new Error('provider not supported.');
    }

    if (!providerTypes.POLYGONSCAN_TYPES.has(type)) {
      throw new Error('type not supported.');
    }

    var subdomain = providerTypes.POLYGONSCAN_NETWORK_TYPES.get(networkType);
    var typePart = providerTypes.POLYGONSCAN_TYPES.get(type);
    return "https://".concat(subdomain, "polygonscan.com/").concat(typePart, "/").concat(value);
  },
  arbiscan: function arbiscan(_ref4) {
    var type = _ref4.type,
        value = _ref4.value,
    ...
    if (!providerTypes.ARBISCAN_NETWORK_TYPES.has(networkType)) {
      throw new Error('provider not supported.');
    }

    if (!providerTypes.ARBISCAN_TYPES.has(type)) {
      throw new Error('type not supported.');
    }

    var subdomain = providerTypes.ARBISCAN_NETWORK_TYPES.get(networkType);
    var typePart = providerTypes.ARBISCAN_TYPES.get(type);
    return "https://".concat(subdomain, "arbiscan.com/").concat(typePart, "/").concat(value);
  }
};
/**
...
function blockExplorerUrl(type, value) {
  var _ref5 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {},
      _ref5$networkType = _ref5.networkType,
      networkType = _ref5$networkType === void 0 ? 'xdai' : _ref5$networkType,
      _ref5$provider = _ref5.provider,
      provider = _ref5$provider === void 0 ? 'blockscout' : _ref5$provider;
