...
export function round(num: number, places = 2) {
  const rounded = Number(Math.round(Number(num + 'e+' + places)) + 'e-' + places);
 ...
 */
export function splitDecimalNumber(num: string) {
 ...
 */
export function toDecimals(num: string, decimals: number, { truncate = true } = {}) {
